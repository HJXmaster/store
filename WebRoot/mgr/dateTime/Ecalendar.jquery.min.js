$.fn.ECalendar = function(A) {
	...
	A = $.extend(f, A);
	var b = $(this), z, q, j, t, n, v, r, H, k, L, m, i, K, h, o, d, D, B, g, e, u = A.step, c, a, E, s = false, l, C, w = A.format ? A.format
			: "yyyy-mm-dd hh:ii", F = A.format ? A.format : "yyyy-mm-dd", G = [
			"#ee3f2f", "#1bbc9b", "#297fb8", "#985aa6", "#e67f22", "#2d3e50",
			"#27ae61", "#7e8c8d" ];
	...
	H.children("li").click(
			function() {
				...
				if (y === 1) {
					p(new Date(M.getFullYear(), M.getMonth(), M.getDate() + 7,
							M.getHours(), M.getMinutes()))
				}
				if (y === 2) {
					p(new Date(M.getFullYear(), M.getMonth() + 1, M.getDate(),
							M.getHours(), M.getMinutes()))
				}
				if (y === 3) {
					p(new Date(M.getFullYear(), M.getMonth() + 3, M.getDate(),
							M.getHours(), M.getMinutes()))
				}
				if (y === 4) {
					p(new Date(M.getFullYear() + 1, M.getMonth(), M.getDate(),
							M.getHours(), M.getMinutes()))
				}
				H.animate({
					"height" : 0
	...
	q.on("mousedown", "li", function(M) {
		...
		c = $(this);
		if (A.type == 0) {
			x.css("overflow", "visible").append(
					'<div class="at12"><div class="pointer"></div></div>');
			l = x.find(".pointer");
			D = M.pageX;
			B = M.pageY;
			...
			c.attr("data-settime", (K < 10 ? "0" + K : K) + ":"
					+ (h < 10 ? "0" + h : h));
			var y = K * 30;
			var y = y + (h / 2);
			l.css({
	...
	$(j).mousedown(
			function(M) {
				...
				var x = '<i class="r3" data="' + (E - 3) + '">' + (E - 3)
						+ "</i>" + '<i class="r2" data="' + (E - 2) + '">'
						+ (E - 2) + "</i>" + '<i class="r1" data="' + (E - 1)
						+ '">' + (E - 1) + "</i>";
				var y = '<i class="a1" data="' + (E + 1) + '">' + (E + 1)
						+ "</i>" + '<i class="a2" data="' + (E + 2) + '">'
						+ (E + 2) + "</i>" + '<i class="a3" data="' + (E + 3)
						+ '">' + (E + 3) + "</i>";
				c.prepend(x);
				c.append(y);
				...
				if (c) {
					...
					if (c[0].tagName == "LI") {
						if (Math.abs(e) > 1 || Math.abs(g) > 1) {
							...
							K = N;
							h = y;
							c.attr("data-settime", (N < 10 ? "0" + N : N) + ":"
									+ (y < 10 ? "0" + y : y));
							r.text((N < 10 ? "0" + N : N) + ":"
									+ (y < 10 ? "0" + y : y));
							l.css({
	...
	function J() {
		...
		a.attr("name", M);
		b
				.after('<svg t="1493098552635" class="icon" style="position: absolute;right:'
						+ (b.height() - 20)
						/ 2
						+ 'px;top:50%;margin-top: -10px;" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="1006" xmlns:xlink="http://www.w3.org/1999/xlink" width="20" height="20"><defs><style type="text/css"></style></defs><path d="M856.576 200.192v54.784h58.368V814.08H109.568V256.512h56.832v-56.32H51.2V870.4h921.6V200.192h-116.224z" fill="#666666" p-id="1007"></path><path d="M819.2 102.4h-204.288v255.488L819.2 358.4V102.4z" fill="#EF4848" p-id="1008"></path><path d="M563.712 664.064h256V716.8h-256zM563.712 512h256v51.2h-256zM461.312 200.192h102.4v56.32h-102.4zM205.312 664.064h256V716.8h-256zM205.312 512h256v51.2h-256z" fill="#666666" p-id="1009"></path><path d="M410.112 102.4H204.8v256l205.312-0.512V102.4z" fill="#EF4848" p-id="1010"></path></svg>');
		b.after(z);
	...
	function p(M) {
		...
		if (z.data("date") != (k + "-" + L)) {
			...
			for (var N = 1; N <= R; N++) {
				if (N < T) {
					Q += "<li class='other prve'>" + (O - (T - N) + 1)
							+ "</li>"
				} else {
					if (N >= T && N < P + T) {
						if ((N - T + 1) == m) {
							Q += "<li class='activ' data='" + (N - T + 1)
									+ "'>" + (N - T + 1) + "</li>"
						} else {
							Q += "<li data='" + (N - T + 1) + "'>"
									+ (N - T + 1) + "</li>"
		...
		q.children("li[data=" + m + "]").addClass("activ").css("background",
				A.skin);
		var x = new Date();
		var S = new Date(k, L - 1, m).getTime()
		...
		var y = A.format.replace("yyyy", k).replace("mm", L).replace("dd", m)
				.replace("hh", K).replace("ii", h).replace('ss',Math.floor(Math.random() * 60));
		b.attr("data-ec", M);
		b.val(y);
		stamp = y.replace(/-/g, "/");
	...
	function I() {
		...
		var N = [ "Android", "iPhone", "SymbianOS", "Windows Phone", "iPad",
				"iPod" ];
		var y = true;
